const detectWorkerCount = () => {
  const envValue = Number(import.meta.env?.VITE_ANALYSIS_WORKER_CONCURRENCY)
...
const createWorkerInstance = () =>
  new Worker(new URL('../../workers/analyze.worker.js', import.meta.url), { type: 'module' })
...
const handleWorkerMessage = (instance, event) => {
  ...
  else job.reject(error ? new Error(error) : new Error('analysis worker failed'))
  assignJobs()
}
...
const handleWorkerError = (instance, event) => {
  ...
  instance.currentJob = null
  if (job) {
    job.reject(event?.error || new Error(event?.message || 'analysis worker error'))
  }
  try {
...
const spawnWorker = () => {
  try {
    ...
    console.error('[analyze] failed to spawn worker', error)
  }
}
...
export const enqueueAnalysisJob = ({ action, payload }) => {
  ensurePool()
  const id = ++jobSeq
