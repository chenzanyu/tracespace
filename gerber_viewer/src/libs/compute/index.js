...
export const enqueueComputeProjectSummary = ({
  projectId,
  layers,
  plotTreesById,
  fileUnits = 'mm',
  ...
  for (const layer of layerList) {
    ...
    const workerIndex = assignWorkerIndex(workerCount, layer.type, layer.filename)
    currentProject.workerIndexByLayerId.set(layer.id, workerIndex)
  }
  ...
  const outlineLayer = layerList.find((layer) => normalizeType(layer?.type) === 'outline')
  const outlineLayerId = outlineLayer?.id || null

  const units = fileUnits === 'in' ? 'in' : 'mm'
  const mmToUnits = (mm) => (units === 'mm' ? mm : mm / 25.4)
  const maxGapUnits = units === 'mm' ? mmToUnits(0.5) : 0.02

  const limit = Number(drillLimit)
  const drillLimitValue = Number.isFinite(limit) && limit > 0 ? limit : Infinity
