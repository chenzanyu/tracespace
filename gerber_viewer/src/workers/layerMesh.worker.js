...
const ACTION_BUILD_LAYER_MESH = 'build-layer-mesh'
...
const buildLayerMeshChunks = (tree, ctx) => {
...
self.onmessage = (event) => {
  ...
  if (action !== ACTION_BUILD_LAYER_MESH) return
  try {
    ...
    const tree = payload?.plotTree
    const chunks = buildLayerMeshChunks(tree, ctx)
    ...
    self.postMessage(
      {
        jobId,
        success: true,
        result: { layerId: payload?.layerId ?? null, chunks },
      },
