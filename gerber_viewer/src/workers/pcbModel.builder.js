...
const boundsIntersect = (bounds, minX, minY, maxX, maxY) => {
  if (!bounds || minX === null || minY === null || maxX === null || maxY === null) {
    return true
  }
...
const elementInsideClip = (element, polygons) => {
  ...
  return points.some((point) => polygons.some((polygon) => pointInPolygon(point, polygon)))
}
...
const filterImageTree = (imageTree, bounds, clipPolygons) => {
  ...
  const filteredChildren = imageTree.children.filter((child) => {
    ...
    if (useBounds && !boundsIntersect(childBounds, minX, minY, maxX, maxY)) {
      return false
    }
...
export const buildPcbLayerPayload = (payload) => {
  const contextBase = {
  ...
  try {
    ...
    const boardClipPolygons = !hasSyntheticOutline
    ...
    const group = measureWorkerStage(metrics, 'render-three', () =>
      renderThree(
        imageTree,
